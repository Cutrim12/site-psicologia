...
if (isset($_POST['enviar'])) {
    ...
    // Esta tela cadastra exclusivamente adultos; não depende de um valor enviado pelo navegador.
    $id_tipo_paciente = 1;
    ...
    <div class="container">
        ...
        <form method="post">
            ...
            <select id="tipo_paciente" name="tipo_paciente" required>
                <option value="" disabled selected>Tipo paciente</option>
                <?php
                    $sql_tipo_paciente = "SELECT * FROM tbl_tipo_paciente";
                    $result_tipo_paciente = mysqli_query($mysqli, $sql_tipo_paciente);
                    while ($row = mysqli_fetch_assoc($result_tipo_paciente)) {
                ?>
