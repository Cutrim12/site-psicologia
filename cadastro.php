...
if (isset($_POST['enviar'])) {
    try {
        ...
        $id_genero = (int) $_POST['sexo'];
        $nascimento = $_POST['nascimento'];
        $logradouro = trim($_POST['endereco']);
        $id_escolaridade = (int) $_POST['escolaridade'];
        $profissao = trim($_POST['profissao']);
        $id_renda_familiar = (int) $_POST['renda_familiar'];
        $rg = trim($_POST['rg']);
        $cpf = trim($_POST['cpf']);
        $id_estado_civil = (int) $_POST['estado_civil'];
        ...
        // Contato do adulto
        $stmt_id_contato = $mysqli->prepare("INSERT INTO tbl_contato (email, telefone_residencial, telefone_recado, celular) VALUES (?, ?, ?, ?)");
        ...
        $stmt_id_profissao = $mysqli->prepare("INSERT INTO tbl_profissao (profissao) VALUES (?)");
        $stmt_id_profissao->bind_param("s", $profissao);
        $stmt_id_profissao->execute();
        $id_profissao = $mysqli->insert_id;
        $stmt_id_profissao->close();

        $stmt_id_bairro = $mysqli->prepare("INSERT INTO tbl_bairro (bairro) VALUES (?)");
        $stmt_id_bairro->bind_param("s", $bairro);
        $stmt_id_bairro->execute();
        $id_bairro = $mysqli->insert_id;
        $stmt_id_bairro->close();

        $stmt_id_logradouro = $mysqli->prepare("INSERT INTO tbl_logradouro (logradouro) VALUES (?)");
        $stmt_id_logradouro->bind_param("s", $logradouro);
        $stmt_id_logradouro->execute();
        $id_logradouro = $mysqli->insert_id;
        $stmt_id_logradouro->close();

        $stmt_id_endereco = $mysqli->prepare("INSERT INTO tbl_endereco (id_bairro, id_logradouro, cep) VALUES (?, ?, ?)");
        ...
        $stmt_id_paciente = $mysqli->prepare("INSERT INTO tbl_paciente (nome, nascimento, rg, cpf, id_tipo_paciente, id_genero, id_contato, id_escolaridade, id_profissao, id_renda_familiar, id_estado_civil, id_endereco, id_maturidade, nome_responsavel) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                <form method="post">
                        ...
                        <div class="textfield-group-3">
                            ...
                            <div class="textfield-2">
                                ...
                                <input type="text" id="rg" name="rg" placeholder="RG ORGÃO EXPEDIDOR" maxlength="20" required>
                            </div>

                            <div class="textfield-2">
                                <label for="cpf">CPF</label>
                                <input type="text" id="cpf" name="cpf" placeholder="CPF" inputmode="numeric" maxlength="14" required>
                            </div>
                        </div>
                        ...
                        <button type="submit" name="enviar" class="btn-cadastro">Cadastrar</button>
